...
public:
   ...
   std::string Generate_GPU_Kernel_ALPAKA(std::string /*opName*/) override {
      ...
      op += SP + SP + "std::size_t numElements, std::size_t axisSize, std::size_t axisStride) const {\n";
      op += SP + SP + SP + "auto idx = alpaka::getIdx<alpaka::Grid, alpaka::Threads>(acc)[0];\n";
      op += SP + SP + SP + "if (idx >= numElements) return;\n";
      op += SP + SP + SP + "std::size_t outer  = idx / (axisSize * axisStride);\n";
      op += SP + SP + SP + "std::size_t inner  = idx % axisStride;\n";
      ...
      op += SP + SP + SP + SP + "sum += exp(data[base + i * axisStride] - vmax);\n";
      op += SP + SP + SP + "out[idx] = exp(data[idx] - vmax) / sum;\n";
      op += SP + SP + "}\n";
      op += SP + "};\n";
      return op;
   }
   ...
   std::string Generate_GPU_ALPAKA(std::string OpName) override {
      ...
      out << SP << "auto const workDiv_" << fNX << " = alpaka::getValidWorkDiv(kernelCfg_" << fNX
         ...
         << "), static_cast<Idx>(" << length << "), static_cast<Idx>(" << axisSize
